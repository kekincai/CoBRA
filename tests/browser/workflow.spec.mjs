...
test("three analytical fixtures → calibration → LOOCV; form id must not be clobbered", async ({
  page,
}) => {
  const errors = [];
  ...
  for (const i of [1, 2, 3]) {
    ...
    await page
      .getByRole("textbox", {
        name: "Project ID 英数字・ハイフン・アンダースコア",
      })
      .fill("ci-fixture-" + i);
    await page
      ...
      .fill("CI TEST ONLY " + i);
    await page
      .getByRole("spinbutton", { name: "開発規模", exact: true })
      .fill(String(i * 100));
    await page
      .getByRole("spinbutton", { name: "実績工数 / 人月 固定工数を含む総工数" })
      .fill(String(i * 20 + 2));
    await page
      .getByRole("spinbutton", { name: "うち固定工数 / 人月" })
      .fill("2");
    ...
    await expect(
      page.getByRole("cell", { name: "CI TEST ONLY " + i, exact: false }),
    ).toBeVisible();
  }
  await page.getByRole("link", { name: "モデル校正", exact: true }).click();
  await page
    .getByRole("textbox", { name: "モデル名", exact: true })
    .fill("CI analytical model");
  ...
  await expect(
    page.getByRole("heading", { name: "案件別の検証結果" }),
  ).toBeVisible();
...
test("official sample → saved estimate → exact replay → CDF → AI comparison", async ({
  page,
}) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  ...
  await page
    .getByRole("button", { name: "シミュレーションを実行・保存" })
    .click();
  await expect(
    page.getByRole("heading", { name: "公開サンプル検証見積り" }),
  ).toBeVisible();
...
test("expert validation and immutable driver version", async ({ page }) => {
  ...
  await page
    ...
    .fill("10");
  await page
    .getByRole("spinbutton", { name: "mode / %", exact: true })
    .fill("22");
  await page
    .getByRole("button", { name: "新しい版として保存", exact: true })
    .click();
  await expect(page.getByRole("dialog")).not.toBeVisible();
  await expect(
    page.getByRole("row").filter({ hasText: "PMのコミュニケーション能力" }),
  ).toContainText("22.0%");
  await page.goto("/drivers?workspace=sample");
  const sampleRow = page
    .getByRole("row")
    .filter({ hasText: "PMのコミュニケーション能力" });
  await expect(sampleRow).toContainText("21.0%");
  await sampleRow.getByRole("button").click();
  await expect(page.getByRole("dialog")).toContainText("読み取り専用");
  await expect(
    page.getByRole("button", { name: "新しい版として保存", exact: true }),
  ).toHaveCount(0);
});

test("benchmark missing data, source cells, and responsive layout", async ({
  page,
}) => {
  await page.goto("/benchmark");
  await expect(
    page.getByRole("heading", { name: "自社実績と日本市場" }),
  ).toBeVisible();
  await expect(page.getByRole("table").first()).toContainText("3.7641");
  await expect(page.getByRole("table").first()).toContainText("0.1513");
  await page
    .getByRole("combobox", { name: "業種", exact: true })
    .selectOption("製造業");
